...
class LelerestapiPwexpressOrderModuleFrontController extends ModuleFrontController
{
    ...
    public function __construct()
    {
        ...
        $this->cart2 = new Cart($this->user['cart_id']);
        $this->customer2 = new Customer($this->user['id_customer']);
        if (!$this->cart2->id && !$this->customer2->id) $this->ajaxDie(Tools::jsonEncode('Not Allowed'));
    }

    public function display()
    {
        $this->createAddress();
        $this->cart2->id_address_delivery = $this->address->id;
        $this->cart2->id_address_invoice  = $this->address->id;

        $this->cart2->save();
        if ($this->placeOrder()) $this->result['success'] = 1;
        else $this->result['success'] = 0;
    ...
    public function createAddress() {
        ...
        $this->address->alias        = $this->getDefaultValue('alias', 'Address');
        $this->address->phone_mobile = Tools::getValue('phone');
        $this->address->lastname     = $this->customer2->firstname;
        $this->address->firstname    = $this->customer2->lastname;
        $this->address->phone        = Tools::getValue('phone', null);
        $this->address->id_customer  = $this->customer2->id;
        $this->address->city         = Tools::getValue('city', null);
        $this->address->id_country   = Tools::getValue('country', 1);
        $this->address->id_state = 1;
        $this->address->other    = "";
        $this->address->postcode = Tools::getValue('zip');
        $this->address->company  = "a";
        $this->address->address1 = Tools::getValue('address');
        $this->address->address2 = Tools::getValue('address');
        ...
        // }
        foreach($this->address->validateController() as $field => $error){
            $this->address->$field = $this->getDefaultValue($field, 'Address');
        }
        if(empty($this->address->id_country))
        $this->address->id_country = $this->getDefaultValue('id_country', 'Address'); //to have delivery
        $this->address->save();
        return $this->address;
    }
    ...
    public function validateOrder(
        ...
        if (!Validate::isLoadedObject($order_status)) {
            ...
            $this->errors[] = "Can't load Order status";
        }
           ...
           // }
            ...
            if ($order_status->logable) {
                ...
                if (!$order->addOrderPayment($amount_paid, null, $transaction_id)) {
				     ...
				     $this->errors[] = 'Cannot save Order Payment';
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                   ...
                   // $error = $this->trans('Order creation failed', array(), 'Admin.Payment.Notification');
				      $this->errors[] = 'Order creation failed';
            ...
            $cart->id_address_invoice = $this->address->id;
            $cart->id_customer = (int) $this->customer2->id;
            $cart->secure_key = $this->customer2->secure_key;
            $cart->id_currency = $this->context->currency->id;
            $cart->id_shop = $this->context->cart->id_shop;
            $cart->save();

            return true;
        } else {
            $this->errors[] = 'Order has already been placed using this cart';
            $cart = new Cart();
            $cart->id_carrier = 0;
            $cart->setDeliveryOption(null);
            $cart->id_address_delivery = $this->address->id;
    ...
    protected function createOrderFromCart(
        ...
        $order_detail->createList($order, $cart, 10, $order->product_list, 0, true, $warehouseId);
